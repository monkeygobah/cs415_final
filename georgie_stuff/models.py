...
def engine(training_stacked, training_labels, data_train, data_test,extractor='sift',  k_predicting=False, bow=True, tfidf= False):
    ...
    performance_metrics_svc_tf = {}
    performance_metrics_rf_tf= {}
    performance_metrics_nb_tf = {}
    performance_metrics_mlp_tf = {}
       ...
       # # Perform k-means on the dataset
        ...
        if bow:
            ...
            if tfidf:
                ...
                train_idf_scores = assessment.calculate_idf(train_histograms, k)
                train_tfidf_histograms = assessment.calculate_tfidf_histograms(train_histograms, train_idf_scores)
                test_tfidf_histograms = assessment.calculate_tfidf_histograms(test_histograms, train_idf_scores)
                train_and_evaluate_model('svc',train_tfidf_histograms, test_tfidf_histograms, data_train['label'], data_test['label'], performance_metrics_svc_tf, k)
                train_and_evaluate_model('random_forest',train_tfidf_histograms, test_tfidf_histograms, data_train['label'], data_test['label'], performance_metrics_rf_tf, k)
                train_and_evaluate_model('naive_bayes',train_tfidf_histograms,test_tfidf_histograms, data_train['label'], data_test['label'], performance_metrics_nb_tf,k)
                train_and_evaluate_model('mlp',train_tfidf_histograms,test_tfidf_histograms, data_train['label'], data_test['label'], performance_metrics_mlp_tf,k)
